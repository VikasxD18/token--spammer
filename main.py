...
#   headers = {"Authorization": token}
  ...
  r = client.post(f"https://canary.discord.com/api/v9/channels/{channel_id}/messages", headers=headers, json={"content": message})
  if r.status_code in (200, 201, 204):
    print(f"Sent message to {channel_id} successfully")
  else:
    print(f"Failed to send message to {channel_id} - {r.text}")


f = open("tokens.txt", "r").readlines()
amount = int(input("[!] Amount: "))
channelx = input("[!] Channel ID: ")
...
#   token = token.split(":")[2]
  ...
  if sent >= amount:
    break 
  time.sleep(0.01)
  Thread(target=send_message, args=(token, channelx,message,)).start()